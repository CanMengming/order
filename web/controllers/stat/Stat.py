...
from common.libs.Helper import ops_render, getFormatDate, iPagination, getDictFilterField, selectFilterObject
...
def food():
    ...
    date_list = []
    if list:
        food_map = getDictFilterField(Food, Food.id, "id", selectFilterObject(list, "food_id"))
        for item in list:
...
def member():
    ...
    date_list = []
    if list:
        member_map = getDictFilterField(Member, Member.id, "id", selectFilterObject(list, "member_id"))
        for item in list:
