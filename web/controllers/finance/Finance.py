...
from common.libs.Helper import ops_render, getCurrentTime
from common.libs.Helper import iPagination, selectFilterObject, getDictListFilterField, getDictFilterField
...
def index():
    ...
    data_list = []
    if pay_list:
        pay_order_ids = selectFilterObject(pay_list, "id")
        ...
        if pay_order_items_map:
            ...
            for item in pay_order_items_map:
                tmp_food_ids = selectFilterObject(pay_order_items_map[item], "food_id")
...
def info():
    ...
    if order_item_list:
        food_map = getDictFilterField(Food, Food.id, "id", selectFilterObject(order_item_list, "food_id"))
...
def set():
    ...
    stat_info = db.session.query(PayOrder, func.sum(PayOrder.total_price).label("total")).filter(PayOrder.status == 1)\
        .first()
